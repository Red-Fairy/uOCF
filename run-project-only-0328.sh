...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 1000 --n_img_each_scene 3  \
    ...
    --model 'uorf_nogan_T' \
    --exp_id '0328-project-only-loadEncoder' --attn_iter 4 \
    --project --relative_position \
    --continue_train --epoch_count 255 --epoch 255 \
    --pos_emb --emb_path /viscam/u/redfairy/I-uORF/checkpoints/clevr_567/uORF-pretrained/latest_net_Encoder.pth \
    --dummy_info 'share grid embed projection, no deduction, add decoder MLP to z-slots projection (w/ residual), 4 round attn, lr 3e-4, pos embedding on encoder (+4), load pretrained encoder with lr /=5' \
