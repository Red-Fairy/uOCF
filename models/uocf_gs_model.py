...
# SlotAttention
from .model_general import MultiDINOStackEncoder, DecoderIPE
from .transformer_attn import SlotAttentionTransformer
from .utils import *
import numpy as np
from .gaussian_decoder import GaussianDecoder, render

class uocfGSModel(BaseModel):
	...
	def modify_commandline_options(parser, is_train=True):
		...
		parser.add_argument('--nss_scale', type=float, default=1., help='Scale of the scene, related to camera matrix')
		...
		parser.add_argument('--n_samples_fg', type=int, default=1000, help='num of samples for each object')
		parser.add_argument('--n_samples_bg', type=int, default=1000, help='num of samples for background')
		...
		parser.add_argument('--fg_boundary', type=float, default=3.0)
		parser.add_argument('--bg_boundary', type=float, default=10.0)
	...
	def __init__(self, opt):
		...
		self.netDecoder = GaussianDecoder(n_freq=opt.n_freq, n_samples_fg=opt.n_samples_fg, n_samples_bg=opt.n_samples_bg, 
										fg_boundary=opt.fg_boundary, bg_boundary=opt.bg_boundary, z_dim=z_dim)
	...
	def set_input(self, input):
		...
		self.full_proj = input['full_proj'].to(self.device)
