...
class uocfDualDINOSingleModel(BaseModel):
	...
	def modify_commandline_options(parser, is_train=True):
		...
		parser.add_argument('--multiview_loss', action='store_true', help='use multiview loss')
		parser.add_argument('--multiview_in', type=int, default=0, help='when to start the multiview loss')
		parser.add_argument('--weight_multiview', type=float, default=0.05, help='weight of the multiview loss')
	...
	def __init__(self, opt):
		...
		if self.opt.multiview_loss:
			self.loss_names += ['multiview']
	...
	def set_visual_names(self, set_depth=False):
		...
		self.visual_names = ['x{}'.format(i) for i in [0]] + \
		...
		if set_depth:
			self.visual_names += ['disparity_{}'.format(i) for i in [0]] + \
	...
	def forward(self, epoch=0):
		...
		self.loss_recon, self.loss_perc, \
			self.loss_bg_density, self.loss_depth_ranking, self.loss_multiview = 0, 0, 0, 0, 0
		...
		raws, masked_raws, unmasked_raws, masks = self.netDecoder(mean, var, z_slots, nss2cam0, fg_slot_nss_position, 
							...
							add_blob=self.collapse_prevent_iter > 0)
		...
		self.loss_recon = self.L2_loss(x_recon[0:1], x[0:1]) # only first view have ground truth
		x_norm, rendered_norm = self.vgg_norm((x[0:1] + 1) / 2), self.vgg_norm(rendered[0:1])
		...
		if self.opt.bg_density_loss and epoch >= self.opt.bg_density_in:
			...
			bg_density = masks[0, ..., -1].flatten(start_dim=0, end_dim=1)  # (NxHxW)xD
		...
		if self.opt.depth_supervision:
			...
			disparity_rendered = 1 / depth_rendered # Nx1xHxW, N=1
			if epoch >= self.opt.depth_in:
				...
				disparity_gt_1 = disparity[0:1, 0, patch1_start_h:patch1_start_h+L, patch1_start_w:patch1_start_w+L].flatten() # N*L**2
				disparity_gt_2 = disparity[0:1, 0, patch2_start_h:patch2_start_h+L, patch2_start_w:patch2_start_w+L].flatten() # N*L**2
				mask_gt = (disparity_gt_1 < disparity_gt_2).float()

				depth_rendered_1 = depth_rendered[0:1, 0, patch1_start_h:patch1_start_h+L, patch1_start_w:patch1_start_w+L].flatten() # N*L**2
				depth_rendered_2 = depth_rendered[0:1, 0, patch2_start_h:patch2_start_h+L, patch2_start_w:patch2_start_w+L].flatten() # N*L**2
		...
		if self.opt.multiview_loss and epoch >= self.opt.multiview_in:
			rendered_slots = torch.zeros([N*self.num_slots, 3, H, W]).to(self.device)
			for k in range(0, self.num_slots):
				raws_slot = unmasked_raws[k]  # NxHxWxDx4
				raws_slot = raws_slot.flatten(start_dim=0, end_dim=2)  # (NxHxW)xDx4
				rgb_map_slot, _ , _ = raw2outputs(raws_slot, self.z_vals, self.ray_dir, mip=True)
				rendered_slot = rgb_map_slot.view(N, H, W, 3).permute([0, 3, 1, 2])  # Nx3xHxW [0, 1]
				# x_recon = F.interpolate(x_recon, size=self.opt.encoder_size, mode='bilinear', align_corners=False)
				rendered_slot = TF.normalize(rendered_slot, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) # Nx3xHxW, N=2
				rendered_slots[k*N:(k+1)*N] = rendered_slot

			rendered_slots = F.interpolate(rendered_slots, size=(H+13)//14*14, mode='bilinear', align_corners=False) # Nx3xHxW, N=2*(K-1)
			cls_tokens = self.pretrained_encoder.forward_features(rendered_slots)['x_norm_clstoken'] # (K-1)*Nx768
			cls_tokens = cls_tokens.view(N, self.num_slots, -1)
			# calculate the similarity between the two views
			self.loss_multiview += (1 - torch.nn.functional.cosine_similarity(cls_tokens[0], cls_tokens[1], dim=-1)).mean() * self.opt.weight_multiview
			# print('multiview loss:', self.loss_multiview.item())
		...
		with torch.no_grad():
			...
			setattr(self, 'x0', x[0])
			if self.opt.depth_supervision:
				setattr(self, 'disparity_0', (disparity[0] - disparity[0].min()) / (disparity[0].max() - disparity[0].min()))
			for i in range(0, self.opt.n_img_each_scene):
				# setattr(self, 'x{}'.format(i), x[i])
				setattr(self, 'x_rec{}'.format(i), x_recon[i])
				if self.opt.depth_supervision: # normalize to 0-1
	...
	def backward(self):
		...
		loss = self.loss_recon + self.loss_perc + self.loss_multiview + self.loss_bg_density + self.loss_depth_ranking 
		loss.backward()
