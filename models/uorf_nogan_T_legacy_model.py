...
import os
import time
from .projection_legacy import Projection
from torchvision.transforms import Normalize
from .model_T_legacy import Encoder, Decoder, SlotAttention, get_perceptual_net, raw2outputs
from .projection import pixel2world


class uorfNoGanTlegacyModel(BaseModel):
	...
	def __init__(self, opt):
		...
		self.netDecoder = networks.init_net(Decoder(n_freq=opt.n_freq, input_dim=6*opt.n_freq+3+z_dim, z_dim=opt.z_dim, n_layers=opt.n_layer,
													locality_ratio=opt.obj_scale/opt.nss_scale, fixed_locality=opt.fixed_locality, project_position=opt.project), gpu_ids=self.gpu_ids, init_type='xavier')
