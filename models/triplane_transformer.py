...
class SlotAttentionTransformer(nn.Module):
	def __init__(self, num_slots, in_dim=64, slot_dim=64, color_dim=8, iters=4, eps=1e-8,
		...
		DINO_dim = 768
		self.out_conv = nn.Sequential(nn.Conv2d(slot_dim+color_dim+1, 128, 3, padding=1), 
									nn.GELU(),
									nn.Conv2d(128, DINO_dim, 3, padding=1))
		self.out_cls_token = nn.Sequential(nn.Linear(slot_dim+color_dim, DINO_dim), 
										   nn.LayerNorm(DINO_dim))


	def forward(self, feat, feat_color, camera_modulation, 
				# global_feat, class_token, triplane_generator,
				num_slots=None, remove_duplicate=False,
				):
		"""
		input:
		...
		feat_color = self.norm_feat_color(feat_color) # (B,H,W,C')
		slot_fg_color = torch.einsum('bkn,bnd->bkd', attn_weights_fg, feat_color.flatten(1, 2)) # (B,K-1,N) * (B,N,C') -> (B,K-1,C')
		slot_bg_color = torch.einsum('bn,bnd->bd', attn_weights_bg.squeeze(1), feat_color.flatten(1, 2)).unsqueeze(1) # (B,N) * (B,N,C') -> (B,C'), (B,1,C')
		slot_bg = torch.cat([slot_bg, slot_bg_color], dim=-1) # (B,1,C+C')

		# generate slot-specific features
		slot_spatial_fg = torch.cat([feat_color.unsqueeze(1).repeat(1, K-1-n_remove, 1, 1, 1), 
									slot_fg.unsqueeze(-2).unsqueeze(-2).repeat(1, 1, H, W, 1),
									attn_weights_fg.view(B, K-1-n_remove, H, W, 1)], dim=-1) # (B,K-1,H,W,C+C'+1)
		feats_fg = self.out_conv(slot_spatial_fg.flatten(0,1).permute(0,3,1,2)).permute(0,2,3,1).flatten(1, 2).view(B, K-1-n_remove, N, -1) # (B,K-1-n_remove,H*W,DINO_dim)

		slot_fg = torch.cat([slot_fg, slot_fg_color], dim=-1) # (B,K-1,C+C')
		cls_tokens_fg = self.out_cls_token(slot_fg) # (B,K-1,DINO_dim)
		feats_fg = torch.cat([feats_fg, cls_tokens_fg.unsqueeze(2)], dim=2) # (B,K-1-n_remove,H*W+1,DINO_dim)
