...
# SlotAttention
from .model_general import MultiRouteEncoderSeparate, MultiDINOEncoder, MultiDINOStackEncoder
...
class uocfStackModel(BaseModel):
	...
	def __init__(self, opt):
		...
		if opt.encoder_type == 'DINO':
			...
			dino_dim = 768
			self.netEncoder = MultiDINOStackEncoder(shape_dim=opt.shape_dim, color_dim=opt.color_dim, input_dim=dino_dim, n_feat_layer=1)
		else:
			assert False

		self.netSlotAttention = SlotAttention(num_slots=opt.num_slots, in_dim=opt.shape_dim, 
							  slot_dim=opt.shape_dim, color_dim=opt.color_dim, pos_emb = opt.slot_attn_pos_emb,
	...
	def encode(self, idx=0):
		...
		feature_map_shape, feature_map_color = self.netEncoder(feature_maps[-1:], feature_maps[0])  # Bxshape_dimxHxW, Bxcolor_dimxHxW
