...
import collections
from .utils import conical_frustum_to_gaussian, cylinder_to_gaussian, debug
...
class ProjectionOCT(object):
    ...
    def construct_intrinsic(self, intrinsics):
        n_scene = intrinsics.shape[0]
        self.cam2spixel = torch.zeros(n_scene, 4, 4).to(self.device)
        self.spixel2cam = torch.zeros(n_scene, 4, 4).to(self.device)

        for i, intrinsic in enumerate(intrinsics):

            focal_x = intrinsic[0, 0] * self.frustum_size[0]
            focal_y = intrinsic[1, 1] * self.frustum_size[1]
            bias_x = ((intrinsic[0, 2] + 1) * self.frustum_size[0] - 1.) / 2.
            bias_y = ((intrinsic[1, 2] + 1) * self.frustum_size[1] - 1.) / 2.
            # print(focal_x, focal_y, bias_x, bias_y)
            intrinsic_mat = torch.tensor([[focal_x, 0, bias_x, 0],
                                            [0, focal_y, bias_y, 0],
                                            [0, 0, 1, 0],
                                            [0, 0, 0, 1]]).to(torch.float32)

            self.cam2spixel[i] = intrinsic_mat.to(self.device)
            self.spixel2cam[i] = intrinsic_mat.inverse().to(self.device)

        if self.radii is None:
            # radii is the 2/sqrt(12) width of the pixel in nss world coordinates
            # calculate the distance in world coord between point (0,0) and (0,1) of camera plane
            ps = torch.tensor([[0, 0, 0, 1], [0, 1, 0, 1]]).to(torch.float32).to(self.device)
            ps = torch.matmul(self.spixel2cam[0], ps.t()).t() # 2x4
            ps = ps / ps[:, 3:] # 2x4
            ps = ps[:, :3] # 2x3
            self.radii = torch.norm(ps[1] - ps[0]) / torch.sqrt(torch.tensor(3.)).to(self.device) / self.nss_scale
    ...
    def construct_origin_dir(self, cam2world):
        ...
        Z = torch.ones_like(X)
        pix_coor = torch.stack([Y, X, Z]).to(self.device)  # 3xHxW, 3=xyz
        cam_coor = torch.matmul(self.spixel2cam[:, :3, :3], pix_coor.flatten(start_dim=1).float())  # Nx3x(HxW)
        ray_dir = cam_coor.permute([0, 2, 1]) # Nx(HxW)x3
        # convert to world coord, cam2world[:, :3, :3] is Nx3x3
        ray_dir = torch.matmul(cam2world[:, :3, :3].unsqueeze(1).expand(-1, H * W, -1, -1), ray_dir.unsqueeze(-1)).squeeze(-1)  # Nx(HxW)x3
        ray_dir = ray_dir.flatten(0, 1)  # (NxHxW)x3
        # ray_dir = F.normalize(ray_dir, dim=-1).flatten(0, 1)  # (NxHxW)x3
    ...
    def construct_sampling_coor(self, cam2world, partitioned=False, intrinsics=None, frustum_size=None, stratified=False):
        ...
        W, H, D = self.frustum_size
        pixel_coor = self.construct_frus_coor(stratified=stratified) # 4x(WxHxD)
        frus_cam_coor = torch.matmul(self.spixel2cam, pixel_coor.float())  # N*4x(WxHxD)
        frus_world_coor = torch.matmul(cam2world, frus_cam_coor)  # Nx4x(WxHxD)
        frus_nss_coor = torch.matmul(self.world2nss, frus_world_coor)  # Nx4x(WxHxD)
        frus_nss_coor = frus_nss_coor.view(N, 4, W, H, D).permute([0, 4, 3, 2, 1])  # NxDxHxWx4
        frus_nss_coor = frus_nss_coor[..., :3] / frus_nss_coor[..., 3:]  # NxDxHxWx3
        ...
        z_vals = (frus_cam_coor[0][2] - self.near) / (self.far - self.near)  # (WxHxD) range=[0,1]
        z_vals = z_vals.expand(N, W * H * D)  # Nx(WxHxD)
        ...
        pix_coor = torch.stack([Y, X, Z]).to(self.device)  # 3xHxW, 3=xyz
        cam_coor = torch.matmul(self.spixel2cam[:, :3, :3], pix_coor.flatten(start_dim=1).float())  # Nx3x(HxW)
        ray_dir = cam_coor.permute([0, 2, 1])
        ray_dir = ray_dir.view(N, H, W, 3)
        ...
        if partitioned:
            ...
            ray_dir = ray_dir.flatten(start_dim=0, end_dim=2)  # (NxHxW)x3
