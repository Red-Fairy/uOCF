...
from .model_general import SDEncoder
from .utils import *
from .SD.ldm_extractor import LdmExtractor

import torchvision

class uorfNoGanTSDFGMaskModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if not opt.preextract:
            self.LdmEncoder = LdmExtractor().to(self.device).eval()
        self.netEncoder = networks.init_net(SDEncoder(z_dim=z_dim), gpu_ids=self.gpu_ids, init_type='normal')
        ...
        if self.isTrain:  # only defined during training time
            ...
            # for name, param in chain(self.netEncoder.named_parameters(), self.netSlotAttention.named_parameters(), self.netDecoder.named_parameters()):
    ...
    def set_input(self, input):
        ...
        if self.opt.preextract:
            self.x_feats = [x.to(self.device) for x in input['img_feats']] # list of features
        else:
    ...
    def forward(self, epoch=0):
        ...
        # (NxHxW)x3, (NxHxW)*D, (NxHxW)*D
        rendered = rgb_map.view(N, H, W, 3).permute([0, 3, 1, 2])  # Nx3xHxW
        x_recon = rendered * 2 - 1

        self.loss_recon = self.L2_loss(x_recon, x)
        if epoch >= self.opt.percept_in:
            if self.opt.use_SD_percept:
                # upsample rendered and x to 128x128)
                rendered_up, x_up = F.interpolate(rendered, size=[128, 128], mode='bilinear', align_corners=False), F.interpolate(x, size=[128, 128], mode='bilinear', align_corners=False)
                rendered_feats, x_feats = self.LdmEncoder({'img': rendered_up, 'text':  ''}), self.LdmEncoder({'img': x_up, 'text': ''})
                for rendered_feat, x_feat in zip(rendered_feats, x_feats):
                    self.loss_perc += self.opt.weight_percept * self.L2_loss(rendered_feat, x_feat)
            else:
