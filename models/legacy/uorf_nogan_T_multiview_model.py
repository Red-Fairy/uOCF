...
from .model_T import Encoder, Decoder, SlotAttention, get_perceptual_net, raw2outputs, Encoder_resnet, set_loss



class uorfNoGanTmultiviewModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--multiview_loss',action='store_true', help='whether to use multiview slot loss')
        parser.add_argument('--weight_multiview', type=float, default=0.2, help='weight for multiview slot loss')
        parser.add_argument('--multiview_in', type=int, default=100)
    ...
    def __init__(self, opt):
        ...
        self.multiview_loss = set_loss() if opt.multiview_loss else None
    ...
    def forward(self, epoch=0):
        # print current LR and last_epoch
        # if self.isTrain:
        #     print(self.optimizers[0].param_groups[-1]['lr'])
        #     print(self.schedulers[0].last_epoch)
        ...
        cam2world_viewer2 = self.cam2world[1]
        ...
        if self.multiview_loss is not None:
            self.loss_multiview = 0
            self.weight_multiview = self.opt.weight_multiview if epoch >= self.opt.multiview_in else 0
            if self.weight_multiview > 0:
                # Encode the second image
                feature_map_view2 = self.netEncoder(F.interpolate(self.x[1:2], size=self.opt.input_size, mode='bilinear', align_corners=False))  # BxCxHxW
                feat_view2 = feature_map_view2.permute([0, 2, 3, 1]).contiguous()  # BxHxWxC
                _, _, fg_slot_position_view2 = self.netSlotAttention(feat_view2)  # 1 x K-1 x 2
                fg_slot_nss_position_view2 = pixel2world(fg_slot_position_view2.squeeze(0), cam2world_viewer2)  # (K-1)x3
                self.loss_multiview = self.multiview_loss(fg_slot_nss_position[:, :2], fg_slot_nss_position_view2[:, :2]) * self.weight_multiview
    ...
    def backward(self):
        ...
        loss = self.loss_recon + self.loss_perc + self.loss_multiview
        loss.backward()
