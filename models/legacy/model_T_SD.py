...
class SlotAttention(nn.Module):
	...
	def forward(self, feat, num_slots=None):
		...
		grid = build_grid(H, W, device=feat.device).flatten(1, 2) # (B,N,2)
		...
		for it in range(self.iters):
			...
			# attn_slotwise = torch.empty(B, K-1, N, device=feat.device)
			...
			for i in range(K):
				if i != 0: # foreground
					...
					attn[:, i] = torch.einsum('bd,bnd->bn', slot_qi, k_i) * self.scale # (B,N)
					# attn_slotwise[:, i-1] = torch.softmax(attn[:, i], dim=-1) # (B,N)
				else: # background
			...
			# update position
			# fg_position = torch.einsum('bkn,bnd->bkd', attn_slotwise, grid) # (B,K-1,N) * (B,N,2) -> (B,K-1,2)
