...
from .model_T_sam_mask import SlotAttention
from .model_general import dualRouteEncoder, sam_encoder
from .triplane import TriplaneRenderer
from .utils import *
from segment_anything import sam_model_registry

import torchvision

class uorfNoGanTriplaneMaskModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netDecoder = networks.init_net(TriplaneRenderer(z_dim=z_dim, triplane_dim=opt.triplane_dim, n_layer=opt.n_layer, locality_ratio=opt.obj_scale/opt.nss_scale, rel_pos=True),
                                            gpu_ids=self.gpu_ids, init_type='normal')
    ...
    def forward(self, epoch=0):
        ...
        sampling_coor_fg = frus_nss_coor.unsqueeze(0).expand(K - 1, -1, -1)  # (K-1)xPx3
        sampling_coor_bg = frus_nss_coor.unsqueeze(0) # 1xPx3

        W, H, D = self.opt.supervision_size, self.opt.supervision_size, self.opt.n_samp
        raws, masked_raws, unmasked_raws, masks = self.netDecoder(sampling_coor_bg, sampling_coor_fg, z_slots, 
                                                                  (self.opt.triplane_resolution, self.opt.triplane_resolution), fg_slot_nss_position)
