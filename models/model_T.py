...
class Encoder(nn.Module):
	...
	def forward(self, x):
		...
		# pixel_emb = build_grid(H, W, x.device)
		# X = torch.linspace(-1, 1, W)
		# Y = torch.linspace(-1, 1, H)
...
class Decoder(nn.Module):
	def __init__(self, n_freq=5, input_dim=33+64, z_dim=64, n_layers=3, locality=True, locality_ratio=4/7, fixed_locality=False, project_position=False):
		"""
		...
		if project_position:
			self.position_projection = nn.Linear(2, z_dim, bias=True)
		else:
	...
	def forward(self, sampling_coor_bg, sampling_coor_fg, z_slots, fg_transform, fg_slot_position, dens_noise=0.):
		...
		if self.position_projection is None:
			...
			# modify the z_slots
			slot_position = torch.cat([fg_slot_position, torch.zeros_like(fg_slot_position[0:1, :])], dim=0)[:, :2] # Kx2
			z_slots = z_slots + self.position_projection(slot_position[:, :2])  # KxC
...
class SlotAttention(nn.Module):
	...
	def forward(self, feat, num_slots=None):
		...
		for it in range(self.iters):
			...
			attn_fg, attn_bg = attn[:, :K-1, :], attn[:, K-1:, :]  # Bx(K-1)xN, Bx1xN
