...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 1000 --n_img_each_scene 3  \
    ...
    --dummy_info 'share grid embed projection, correct deduct operation (before azi transform), move deduction after locality, add decoder MLP to z-slots projection (w/o residual), 4 round attn, pos embedding on encoder (+4), load pretrained encoder with lr *= 0.1, no bg MLP encoding' \
