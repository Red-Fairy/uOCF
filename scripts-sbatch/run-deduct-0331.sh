...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 1000 --n_img_each_scene 3  \
    ...
    --model 'uorf_nogan_T' \
    --exp_id '0331-deduct-only-loadEncoder' --attn_iter 4 \
    ...
    --dummy_info 'share grid embed projection, correct deduct operation (before azi transform), move deduction after locality, 4 round attn, pos embedding on encoder (+4), load pretrained encoder with lr *= 0.1' \
