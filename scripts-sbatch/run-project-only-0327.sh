...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 1000 --n_img_each_scene 4  \
    ...
    --model 'uorf_nogan_T' \
    --exp_id '0327-project-only' --attn_iter 3 \
    --project \
    --relative_position \
    --dummy_info 'new grid, modify attn visuals, share grid embed projection, correct deduct operation (before azi transform), project w/o relative_pos' \
