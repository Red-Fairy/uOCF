...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 1000 --n_img_each_scene 3  \
    ...
    --exp_id '0410-sam-texture-v2' \
    --z_dim 30 --texture_dim 10 \
    --sam_encoder --encoder_size 1024 \
    --attn_iter 4 \
    --seed 1 --lr 1e-4 \
    --project \
    --dummy_info 'dim=30:10, lr 1e-4, frozen sam encoder, disentangle texture, share grid embed projection, correct deduct operation (before azi transform), move deduction after locality, add decoder MLP to z-slots projection (w/ residual), 4 round attn, use ImageNet ResNet18 encoder, pyramid upsample' \
