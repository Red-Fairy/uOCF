...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 324 --n_img_each_scene 2 \
    ...
    --load_pretrain --load_pretrain_path '/viscam/projects/uorf-extension/uOCF/checkpoints/rebuttal/CLEVR2Real/4obj-learn' \
    --load_encoder 'load_train' --load_slotattention 'load_train' --load_decoder 'load_train' \
    --load_epoch 40 \
    --stratified --fg_object_size 3 --fixed_locality --bg_density_loss \
    --n_dense_samp 256 --dense_sample_epoch 150 \
    --learnable_slot_init \
    --exp_id 'CLEVR2Real/4obj-real-r2' \
    --dummy_info '' \
