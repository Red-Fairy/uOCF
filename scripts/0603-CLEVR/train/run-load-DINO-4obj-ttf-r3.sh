...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 1000 --n_img_each_scene 4 \
    ...
    --load_pretrain --load_pretrain_path '/viscam/projects/uorf-extension/I-uORF/checkpoints/clevr_bg/1obj-bg-scratch' \
    --load_encoder 'load_train' --load_slotattention 'load_train' --load_decoder 'load_freeze' \
    --exp_id 'load-DINO-567obj-ttf-r2' \
    --dummy_info 'load DINO from r1 checkpoint (can disentangle fg/bg)' \
