...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 1000 --n_img_each_scene 3  \
    ...
    --model 'uorf_nogan_T' \
    --exp_id '0403-project-loadEncoder-fg' --attn_iter 4 \
    --project \
    ...
    --lr_policy 'warmup' --niter 800 --coarse_epoch 400 \
    --dummy_info 'fg objects in world coord, share grid embed projection, correct deduct operation (before azi transform), move deduction after locality, add decoder MLP to z-slots projection (w/ residual), 4 round attn, pos embedding on encoder (+4), load pretrained encoder with lr *= 0.1, w/ bg MLP encoding' \
