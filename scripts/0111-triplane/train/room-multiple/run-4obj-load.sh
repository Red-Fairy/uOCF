...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 5000 --n_img_each_scene 2 \
    --checkpoints_dir 'checkpoints' --name '0111-triplane' \
    ...
    --scaled_depth  --depth_scale 12.2 --fixed_dist 12.2 --depth_scale_pred \
    --remove_duplicate --remove_duplicate_in 20 --vis_mask \
    --dec_density_act 'softplus' \
    --load_pretrain --load_pretrain_path '/viscam/projects/uorf-extension/uOCF/checkpoints/0111-triplane/room-multiple/1obj-triplaneOnly-predDepth' \
    --load_encoder 'load_train' --load_slotattention 'load_train' --load_decoder 'load_train' --one2four  \
    --dec_density_act 'softplus' \
    --exp_id 'room-multiple/4obj-triplane' \
