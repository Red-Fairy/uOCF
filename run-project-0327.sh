...
python train_without_gan.py --dataroot $DATAROOT --n_scenes 1000 --n_img_each_scene 4  \
    ...
    --load_size 128 --n_samp 64 --input_size 64 --supervision_size 64 --coarse_epoch 600 --z_dim 36 --num_slots 8 \
    --model 'uorf_nogan_T' \
    --exp_id '0327-project' --attn_iter 3 \
    --project \
    --dummy_info 'new grid,  share grid embed projection, correct deduct operation (before azi transform), move deduction after locality, add decoder MLP to z-slots projection (no residual), percept_in 100' \
